...
- Do not suggest lineage for concepts already well-covered by the existing papers."""
    ...
    async def _prompt_json(self, prompt: str, ip: str = "unknown"):
        ...
        try:
            ...
            raise HTTPException(status_code=429, detail="Anthropic rate limit hit. Please wait a moment and try again.") from e

        await limiter.record(ip, resp.usage.input_tokens, resp.usage.output_tokens, self.model)
