...
function existingGraphPapers(data: TimelineData): GraphPaper[] {
  return Object.values(data.nodes).map((node) => ({
    ...
    isOa: node.paper.isOa ?? false,
    oaStatus: node.paper.oaStatus ?? null,
    hasFulltext: node.paper.hasFulltext ?? false,
    hasContentPdf: node.paper.hasContentPdf ?? false,
    hasContentTei: node.paper.hasContentTei ?? false,
    oaLicense: node.paper.oaLicense ?? null,
...
function mergePaper(node: TimelineNode, paper: GraphPaper): TimelineNode["paper"] {
  return {
    ...
    isOa: paper.isOa ?? node.paper.isOa,
    oaStatus: paper.oaStatus ?? node.paper.oaStatus,
    hasFulltext: paper.hasFulltext ?? node.paper.hasFulltext,
    hasContentPdf: paper.hasContentPdf ?? node.paper.hasContentPdf,
    hasContentTei: paper.hasContentTei ?? node.paper.hasContentTei,
    oaLicense: paper.oaLicense ?? node.paper.oaLicense,
...
function preferPaper(current: GraphPaper, candidate: GraphPaper): GraphPaper {
  ...
  return {
    ...
    isOa: winner.isOa ?? loser.isOa,
    oaStatus: winner.oaStatus ?? loser.oaStatus,
    hasFulltext: winner.hasFulltext ?? loser.hasFulltext,
    hasContentPdf: winner.hasContentPdf ?? loser.hasContentPdf,
    hasContentTei: winner.hasContentTei ?? loser.hasContentTei,
    oaLicense: winner.oaLicense ?? loser.oaLicense,
